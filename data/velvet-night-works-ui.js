(() => {
  const LATEST_ID = "velvet-night-amagi-yo";
  const apply = () => {
    ...
    document.querySelectorAll('.work-card').forEach(card => {
      card.classList.toggle('velvet-latest-card', String(card.dataset.id || '') === LATEST_ID);
    });
  ...
  style.textContent = `
    .work-card .work-new-badge{display:none!important}
    .work-card[data-id="${LATEST_ID}"] .work-visual::before{
      content:"NEW RELEASE";position:absolute;z-index:8;right:10px;top:10px;
      padding:8px 11px;border:1px solid rgba(240,220,165,.5);
      background:rgba(9,10,22,.88);color:#f0dca5;
      font:600 8px/1.2 "Cormorant Garamond",serif;letter-spacing:.16em;
